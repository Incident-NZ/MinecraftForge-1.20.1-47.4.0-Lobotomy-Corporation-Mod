...
public class W3Harmony extends CrossbowItem {
    private static final String AMMO_TAG = "match_ammo";
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity entityLiving, int timeLeft) {
        ...
        if (chargeTime >= 100 && getAmmo(stack) == 0) { // 5秒 = 100tick
    ...
    private void shoot(Level level, Player player, ItemStack stack) {
        ...
        Vec3 spawn = player.getEyePosition().add(direction.scale(1.0));

        W2Match.CustomFireball fireball = new W2Match.CustomFireball(level, player, direction.x, direction.y, direction.z);
    ...
    private void setAmmo(ItemStack stack, int value) {
        stack.getOrCreateTag().putInt(AMMO_TAG, Math.max(0, Math.min(MAX_AMMO, value)));
    }
